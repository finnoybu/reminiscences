...
export default function AuthModal({ onClose }: Props) {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    // Wrap in try/finally so the "Please wait..." state always clears even
    // when the Better Auth client throws (e.g. 503 from /api/auth/*).
    // Supabase always returned { error }; Better Auth can throw on transport
    // failures, so the original Next.js pattern was incomplete here.
    try {
      if (view === 'sign_up') {
        ...
        const { error } = await (signIn as any).email({ email, password, callbackURL: '/account' });
        if (error) { setError(error.message ?? 'Could not sign in.'); return; }
        window.location.href = '/account';
        return;
      } else if (view === 'forgot_password') {
        // Better Auth 1.6.11 renamed the endpoint from forget-password →
        // request-password-reset. Use the matching client method.
        const { error } = await (authClient as any).requestPasswordReset({
          email,
          // Land back on home — AuthModalRoot detects ?token=<token> in the URL
          // and pops the recovery modal over whatever was there.
          redirectTo: '/',
        });
        if (error) { setError(error.message ?? 'Could not send the reset link.'); return; }
        setView('check_email');
      }
    } catch (err: any) {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'facebook' | 'apple' | 'github') => {
    ...
    setError(null);
    try {
      const callbackURL = window.location.pathname || '/account';
      // Better Auth returns { error } inline on failures (e.g. provider not
      // registered server-side) rather than throwing — destructure or the
      // modal would hang silently.
      const result: any = await (signIn as any).social({ provider, callbackURL });
      if (result?.error) {
        setError(result.error.message ?? `Could not start ${provider} sign-in.`);
      }
      // Success path: Better Auth redirects to the provider's auth URL itself.
    } catch (err: any) {
          ...
          {view === 'check_email' ? (
            ...
            <>
              {(view === 'sign_in' || view === 'sign_up') && (
                <>
                  {/*
                    Apple Sign In is temporarily removed. Apple's form_post
                    callback is a cross-site POST that strips SameSite=Lax
                    cookies and trips Better Auth's CSRF guard. This naturally
                    resolves once auth is centralized on auth.finnoybu.com
                    (same-site to the session cookie). Restore the Apple
                    button at that point. The three remaining buttons fit a
                    1x3 row better than the original 2x2 grid.
                  */}
