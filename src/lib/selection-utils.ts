// Pure DOM helpers for text-selection mechanics on the reader. Ported
// verbatim from the Next.js main:lib/selection-utils.ts (no platform
// dependencies — TreeWalker math + popover positioning).
...
// Extract selection data relative to an article element. Returns null if
// the selection is empty or outside the article.
export function getSelectionInfo(articleEl: HTMLElement): SelectionInfo | null {
...
// Walk text nodes in document order to compute a character offset from the
// start of the element's textContent.
...
// Returns x (center-aligned, clamped to viewport), y, and whether the popover
